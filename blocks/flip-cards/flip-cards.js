...
function addFlipFunctionality(block) {
  ...
  flipToBackButtons.forEach((button, index) => {
    button.addEventListener('click', (e) => {
      e.preventDefault();
      const flipCardContainer = button.closest('.flip-card-container');
      flipCardContainer.classList.add('flipped');
      acdl.pushEvent('flipCardInteraction', {
        component: {
          componentType: 'flip-cards',
          componentPath: 'blocks/flip-cards',
          cardIndex: index,
          action: 'flip-to-back',
        },
      });
  ...
  flipToFrontButtons.forEach((button, index) => {
    button.addEventListener('click', (e) => {
      e.preventDefault();
      const flipCardContainer = button.closest('.flip-card-container');
...
export default async function decorate(block) {
  ...
  const flipCards = block.querySelectorAll('.flip-card-container');
  acdl.push({
    component: {
      componentType: 'flip-cards',
      componentPath: 'blocks/flip-cards',
      cardCount: flipCards.length,
    },
  });
}

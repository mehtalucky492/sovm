...
const acdl = createAcdlHelper({ debug: false });
...
// not needed since we are going to use martech to load launch
// function addLaunch() {
//   const launch = getMetadata('launch-id');
//   if (launch) {
//     const script = document.createElement('script');
//     script.src = `//assets.adobedtm.com/3f0d2e2dbd42/313fd7baa8af/launch-${launch}.min.js`;
//     script.defer = true;
//     document.head.appendChild(script);
//   }
// }
...
export function decorateButtons(element) {
  element.querySelectorAll('a').forEach((a) => {
    ...
    // ADD TRACKING HERE (once per anchor)
    a.addEventListener('click', () => {
      const isPrimary = a.classList.contains('primary');
      const isSecondary = a.classList.contains('secondary');

      let variant = 'default';
      if (isPrimary) variant = 'primary';
      else if (isSecondary) variant = 'secondary';

      acdl.pushEvent('componentClick', {
        component: {
          componentType: 'button',
          componentTitle: a.textContent.trim(),
...
async function loadBlock(block) {
  ...
  if (status !== 'loading' && status !== 'loaded') {
    ...
    try {
      const cssLoaded = loadCSS(`${codeBasePath}/blocks/${blockName}/${blockName}.css`);
      const decorationComplete = new Promise((resolve) => {
        (async () => {
          try {
            const mod = await import(
              `${codeBasePath}/blocks/${blockName}/${blockName}.js`
            );
            if (mod.default) {
